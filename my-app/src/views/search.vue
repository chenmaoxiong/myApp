<template>
    <div>
        <div>
            <form action="/">
                <van-sticky>
                    <van-search v-model="searchVal" placeholder="搜索门店·内容·用户" show-action shape="round" @search="onSearch" @cancel="onCancel"
                    />
                </van-sticky>
            </form>
        </div>
        <div class="search" v-if="isSearch">
            <div class="hot_search">
                <p>热搜</p>
                <van-button type="primary" size="small" v-for="(item,i) of hot" :key="i">{{item}}</van-button>
            </div>
            <div class="record_search">
                ...
                <van-button type="primary" size="small" v-for="(item,i) of record" :key="i">{{item}}</van-button>
            </div>
        </div>
        <div v-else="isSearch">
    ...
    export default {
        ...
        data() {
            return {
                searchVal: '',
                hot: ['海底捞', '大学生', '美甲'],//热搜
                record: ['海珠广场', '广州', '天河', '广州', '海珠广场', '广州', '海珠广场', '广州'],//搜索记录
                /**搜索完的页面**/
                activeName: 'a',//
                isSearch: true
            }
